...
exports.GoalOptimizationService = void 0;
class GoalOptimizationService {
    /**
     * Calculate optimal monthly contribution for goal
     */
    static calculateOptimalContribution(goal, monthsAvailable) {
     ...
     */
    static analyzeFeasibility(goal, monthlyIncome, otherObligations) {
        const monthsAvailable = Math.max(1, Math.floor((new Date(goal.deadline).getTime() - new Date().getTime()) / (30 * 24 * 60 * 60 * 1000)));
        const needed = goal.targetAmount - goal.currentAmount;
        const monthlyNeeded = Math.ceil(needed / monthsAvailable);
        const availableAfterObligations = monthlyIncome - otherObligations;
        let feasibility;
        if (monthlyNeeded <= availableAfterObligations * 0.2)
            feasibility = 'easy';
        else if (monthlyNeeded <= availableAfterObligations * 0.5)
            feasibility = 'moderate';
        else if (monthlyNeeded <= availableAfterObligations)
     ...
     */
    static createFinancialPlan(goals, monthlyIncome, necessaryExpenses) {
        const availableForGoals = monthlyIncome - necessaryExpenses;
        const optimizations = goals.map(g => this.analyzeFeasibility(g, monthlyIncome, necessaryExpenses));
        // Calculate priority scores
        const priorityScore = {};
        for (const goal of goals) {
            const monthsToDeadline = Math.max(1, Math.floor((new Date(goal.deadline).getTime() - new Date().getTime()) / (30 * 24 * 60 * 60 * 1000)));
        ...
        if (totalNeeded > availableForGoals) {
            recommendations.push(`Your goals require $${totalNeeded.toFixed(2)}/month but you have $${availableForGoals.toFixed(2)} available`);
            recommendations.push(`Focus on priorities: ${sortedGoals.map(g => g.name).join(', ')}`);
        }
        else {
            recommendations.push(`You can afford all goals and have $${(availableForGoals - totalNeeded).toFixed(2)}/month remaining`);
        }
        return {
            goals: sortedGoals,
     ...
     */
    static suggestAdjustments(goal, monthlyIncome, available) {
        if (available <= 0) {
            return {
        ...
        return {
            recommendation: achievableTarget >= goal.targetAmount
                ? 'Goal is achievable'
...
exports.GoalOptimizationService = GoalOptimizationService;
//# sourceMappingURL=goal-optimization.service.js.map